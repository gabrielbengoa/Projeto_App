...
foto6 varchar(200) not null,
foto7 varchar(200) not null,
foto8 varchar(200) not null,
foto9 varchar(200) not null,
foto10 varchar(200) not null,
foto11 varchar(200) not null
)engine InnoDB;
...
insert into foto(foto1,foto2,foto3,foto4) values ('placavideo1.png','placavideo2.png','placavideo3.png','placavideo4.png');
  
insert into produto(nomeproduto, descricao, preco, idfoto)values('PC Gamer','Monitor de 23 polegadas Led',856.90,1);
...
select cl.nomecliente, cl.cpf, cl.sexo, co.telefone, co.email, en.cep, en.numero, us.nomeusuario, us.foto
from cliente cl inner join contato co 
on cl.idcontato = co.idcontato
inner join endereco en
on cl.idendereco = en.idendereco
...
pg.parcelas,
pg.valorparcela
from pedido pe inner join pagamento pg
on pe.idpedido = pg.idpedido
inner join itenspedido it
on pe.idpedido = it.idpedido
inner join produto pr
on it.idproduto = pr.idproduto;

  
  
  select * from cliente;
  
  select p.idproduto,p.nomeproduto, p.preco, f.foto1 from produto p inner join foto f on p.idfoto=f.idfoto;
